...
pub fn piece_iterator<'a>(position: &'a Position0x88) -> impl 'a + Iterator<Item = SquareAndPiece> {
    square_iterator()
...
mod test {
    use crate::{
        fen::STARTPOS_FEN,
        position0x88::{get_piece, BLACK, ROOK, WHITE},
    };
    ...
    fn test_piece_iterator() {
        let position: Position0x88 = STARTPOS_FEN.into();
